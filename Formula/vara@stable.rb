...
class VaraATStable < Formula
  ...
  stable do
    on_arm do
      url "https://get.gear.rs/vara-testnet-aarch64-apple-darwin.tar.xz"
    end

    on_intel do
      url "https://get.gear.rs/vara-testnet-x86_64-apple-darwin.tar.xz"
    end
  end

  def install
    libexec.install Dir["*"]
    bin.install_symlink Dir["#{libexec}/vara"]
  end
